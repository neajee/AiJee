...
export function useConnectionStatusController(): ConnectionStatusBannerViewProps & {
  ...
  const bottomPad = true ? 0 : Math.max(insets.bottom, 6);
  ...
  if (hasConnectionIssue && isReconnecting) {
    message = connection.retryAttempt > 1 ? `Reconnecting… (attempt ${connection.retryAttempt})` : 'Reconnecting…';
    if (isWaitingToRetry) {
      message = connection.retryAttempt > 1 ? `Retrying in ${retryCountdownSeconds}s (attempt ${connection.retryAttempt})` : `Retrying in ${retryCountdownSeconds}s`;
    }
