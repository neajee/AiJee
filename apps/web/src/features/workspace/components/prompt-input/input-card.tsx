...
export function InputCard({
  ...
  >
      ...
      <div className="flex items-center px-2 pb-2">
        ...
        {showQueueActions ? <div className="flex items-center gap-2">{(['steer', 'followUp'] as QueueBehavior[]).map(behavior => <button className={`inline-flex h-7 items-center justify-center rounded px-1.5 text-[12px] font-medium leading-4 transition-colors hover:bg-hover hover:underline disabled:cursor-not-allowed disabled:opacity-45 ${behavior === 'steer' ? 'text-primary' : 'text-text-secondary'}`} key={behavior} role="button" aria-label={`Send as ${queueLabel(behavior)}`} onClick={() => sendDraft(behavior)} disabled={inputDisabled || !hasDraft}><span>{queueLabel(behavior)}</span></button>)}</div> : <button className={`ml-auto inline-flex size-8 items-center justify-center rounded-full hover:opacity-85 disabled:cursor-not-allowed disabled:opacity-45 ${theme.isDark ? 'bg-[#4d4d4b] text-[#fefdfd]' : 'bg-foreground text-background'}`} role="button" aria-label={showAbortButton ? 'Stop generation' : 'Send message'} onClick={handleSubmit} disabled={sendBlocked}>{showAbortButton ? <Square size={12} fill="currentColor" /> : <ArrowUp size={16} strokeWidth={2} />}</button>}
      </div>
    </div>;
}
