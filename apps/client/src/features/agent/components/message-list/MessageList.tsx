...
export function MessageListView({
  ...
  const listHeader = <div className="flex flex-col">
      {session.isLoadingOlderMessages ? <div entering={FadeIn.duration(180)} exiting={FadeOut.duration(180)} className="flex flex-col">
  ...
  return <div className="flex flex-col">
      <VirtualList<ListItem> ref={listRef} data={items} renderItem={renderItem} keyExtractor={keyExtractor} className="flex flex-col" onScroll={handleScroll} onLayout={alignToLatest} onContentSizeChange={handleContentSizeChange} onScrollBeginDrag={handleScrollBeginDrag} onScrollEndDrag={handleScrollEndDrag} onMomentumScrollEnd={handleScrollEndDrag} scrollEventThrottle={16} initialNumToRender={INITIAL_RENDER_COUNT} maxToRenderPerBatch={RENDER_BATCH_COUNT} updateCellsBatchingPeriod={50} windowSize={WINDOW_SIZE} removeClippedSubviews={false} keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive" maintainVisibleContentPosition={historyAnchor ? {
      minIndexForVisible: 0
    } : undefined} ListHeaderComponent={listHeader} />
      {showScrollButton && <div entering={FadeIn.duration(150)} exiting={FadeOut.duration(150)} className="flex flex-col">
