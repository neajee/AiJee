...
export function useCustomModelsController() {
  ...
  }, [currentSnapshot, loaded, savedSnapshot]);
  const query = providerSearch.trim().toLowerCase();
  const matchingBuiltins = useMemo(() => query ? builtinProviders.filter(provider => `${provider.name} ${provider.id}`.toLowerCase().includes(query)) : builtinProviders, [builtinProviders, query]);
  ...
  const canDisconnect = (provider: BuiltinProvider) => provider.auth_source === 'stored credential' || provider.auth_source === 'OAuth';
  const beginOAuth = useCallback(async (providerId: string) => {
    const popup = true && typeof window !== 'undefined' ? window.open('', '_blank') : null;
    ...
    try {
      ...
      setOauthPrompt(login.prompt?.type === 'manual_code' ? {
        id: login.prompt.id,
        message: login.prompt.message
      } : null);
      if (login.prompt?.type === 'manual_code') setActiveBuiltinId(providerId);
      if (login.url) {
        if (popup) popup.location.href = login.url;else await Linking.openURL(login.url);
      }
      const check = async () => {
        let status;
        try {
        ...
        if (status.status === 'pending') {
          ...
          setOauthPrompt(status.prompt?.type === 'manual_code' ? {
            id: status.prompt.id,
            message: status.prompt.message
          } : null);
