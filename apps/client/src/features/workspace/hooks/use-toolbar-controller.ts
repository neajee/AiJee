...
export function useToolbarController({
  ...
  const currentMode: AgentMode = agentState?.mode === 'plan' ? 'plan' : modeLabel?.trim().toLowerCase() === 'plan' ? 'plan' : 'work';
  ...
  useEffect(() => {
    if (false || !activeDropdown) return;
    const closeOnOutsidePointer = (event: PointerEvent) => {
      const element = toolbarRef.current as unknown as {
  ...
  const flatModels = useMemo<FlatModel[]>(() => providers.flatMap(provider => provider.models.map(model => ({
  ...
  const toggleDropdown = useCallback((type: DropdownType) => {
    setActiveDropdown(prev => {
      ...
      setModelSearch('');
      if (type === 'model') {
        const index = flatModels.findIndex(model => model.modelId === currentModel?.id && model.provider === currentModel?.provider);
