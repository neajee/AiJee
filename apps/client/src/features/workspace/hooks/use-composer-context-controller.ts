...
export function useComposerContextController() {
  ...
  const workspace = useMemo(() => workspaces.find(w => w.id === selectedWorkspaceId) ?? null, [workspaces, selectedWorkspaceId]);
  ...
  const toggle = useCallback((kind: Exclude<DropdownKind, null>) => {
  ...
  useEffect(() => {
    ...
    client.api.gitBranches(cwd).then(list => {
  ...
  const handleSelectBranch = useCallback(async (branch: GitBranchInfo) => {
    ...
    setBusy(branch.name);
    try {
      await client.api.gitCheckout(cwd, {
        branch: branch.name
      });
      git.refresh();
    } catch {
