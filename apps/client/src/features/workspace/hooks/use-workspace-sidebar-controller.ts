...
async function loadStoredCollapsed(scope: string): Promise<boolean | null> {
  ...
  try {
    const value = true ? typeof localStorage === 'undefined' ? null : localStorage.getItem(key) : await SecureStore.getItemAsync(key);
...
export function useWorkspaceSidebarController({
  ...
  const {
    isGitRepo
  } = useGitStatus(workspace?.path ?? null);
  ...
  const widthAnim = useRef(new Animated.Value(locked ? 0 : scope.collapsed ? COLLAPSED_WIDTH : scope.width)).current;
  ...
  useEffect(() => {
    ...
    if (scope.widthLoaded) {
      ...
      promises.push(loadStoredWidth(storageScope).then(storedWidth => {
        if (cancelled) return;
    ...
    if (scope.collapsedLoaded) {
      ...
      promises.push(loadStoredCollapsed(storageScope).then(stored => {
        if (cancelled) return;
  ...
  return {
    ...
    toggleCollapsed,
    openPane,
    isDesktopShell: true && typeof navigator !== 'undefined' && navigator.userAgent.includes('AiJeeDesktop/'),
    widthAnim
  };
}
