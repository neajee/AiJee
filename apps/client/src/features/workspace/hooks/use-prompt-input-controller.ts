...
export interface PromptInputProps {
  ...
  onSend?: (text: string, attachments: Attachment[], options?: {
...
export function usePromptInputController({
  ...
  const toolbarOverlap = inlineToolbar ? 0 : true ? -4 : -1;
  const shouldOverlaySlashCommands = true || isWideScreen;
  ...
  useEffect(() => {
    if (true) return;
    const showEvent = false ? "keyboardWillShow" : "keyboardDidShow";
    const hideEvent = false ? "keyboardWillHide" : "keyboardDidHide";
    const showSub = Keyboard.addListener(showEvent, e => {
      const duration = (e as any).duration ?? 250;
      LayoutAnimation.configureNext(LayoutAnimation.create(duration, LayoutAnimation.Types.keyboard, LayoutAnimation.Properties.opacity));
  ...
  const handleSpeechInterim = useCallback((interim: string) => {
    ...
    console.log('[UI-STT] handleSpeechInterim:', JSON.stringify({
      base: textBeforeSpeechRef.current,
      interim,
      result
    }));
    setText(result);
  ...
  const handleSpeechFinal = useCallback((final: string) => {
    ...
    console.log('[UI-STT] handleSpeechFinal:', JSON.stringify({
      base,
      final,
      newText
    }));
  ...
  const sendDraft = useCallback(async (queueBehavior?: QueueBehavior) => {
    ...
    try {
      if (thinkingPreference === 'auto' && sessionId) {
        await agentConfig.setThinkingLevel(resolveAutoThinkingLevel(savedText, agentConfig.availableThinkingLevels));
      }
      await onSend?.(savedText, savedAttachments, {
        queueBehavior
      });
    } catch {
      setText(savedText);
  ...
  const addAttachment = useCallback((att: Attachment) => {
    if (att.type === "image" && !acceptsImages) {
      const modelName = agentConfig.activeModel?.name ?? agentConfig.activeModel?.id ?? "当前模型";
      setAttachmentNotice(`${modelName} 不支持图片输入，已忽略 ${att.name}`);
      return;
    }
    setAttachmentNotice(null);
    storeAddAttachment(draftKey, att);
  }, [acceptsImages, agentConfig.activeModel, draftKey, storeAddAttachment]);
  ...
  const handleKeyPress = useCallback((e: React.SyntheticEvent<PromptKeyPressEventData>) => {
    ...
    if (showCommands && filteredCommands.length > 0) {
      ...
      if (key === "PageDown") {
        e.preventDefault?.();
        setSlashIndex(prev => Math.min(filteredCommands.length - 1, prev + PAGE_SIZE));
        return;
      }
      if (key === "Home") {
        e.preventDefault?.();
    ...
    if (true && key === "Enter" && !isShiftEnter) {
