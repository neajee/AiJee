...
export function ContextDropdown({
  ...
  return <div className={""}>
      <div className={""}>
        ...
        {open === 'project' && <div role="menu" aria-label="Project selection" className={""}>
          <div className={""}>
            ...
            return <button key={workspace.id} onClick={() => onSelectProject(workspace.id)} role="menuitem" accessibilityState={{
              selected: active
                ...
                <div className={""}><div className={"" + " " + ""} /><span className={"" + " " + ""}>{workspace.title}</span></div>
                {active && <Check size={13} color={theme.accentColor} strokeWidth={2} />}
              </button>;
          })}
          </div>
          <button onClick={onAddWorkspace} role="menuitem" aria-label="添加新项目">
            <Plus size={13} color={theme.textMuted} strokeWidth={1.8} /><span className={"" + " " + ""}>添加新项目</span>
          </button>
        </div>}
      </div>
      <div className={""}>
        ...
        {open === 'environment' && <div role="menu" aria-label="Environment selection" className={""}>
          <div className={""}>
            {servers.map(server => {
            ...
            return <button key={server.id} onClick={() => void onSelectServer(server)} role="menuitem" accessibilityState={{
              selected: active
                ...
                <div className={""}><Globe size={13} color={active ? theme.accentColor : theme.textMuted} strokeWidth={1.8} /><div className={""}><span className={"" + " " + ""}>{server.name}</span><span className={"" + " " + ""}>{server.address}</span></div></div>
      ...
      {isGitRepo && <div className={""}>
        ...
        {open === 'branch' && <div role="menu" aria-label="Branch selection" className={""}>
          {branchesLoading && !branches ? <div className={""}><span size="small" color={theme.textMuted} /></div> : localBranches.length === 0 ? <span className={"" + " " + ""}>No branches found</span> : <><div className={""}>{localBranches.map(branch => <button key={branch.name} onClick={() => void onSelectBranch(branch)} role="menuitem" accessibilityState={{
              selected: branch.is_current
            }}><div className={""}><GitBranch size={13} color={branch.is_current ? theme.accentColor : theme.textMuted} strokeWidth={1.8} /><span className={"" + " " + ""}>{branch.name}</span></div>{busy === branch.name ? <span size="small" color={theme.textMuted} /> : branch.is_current && <Check size={13} color={theme.accentColor} strokeWidth={2} />}</button>)}</div><button onClick={onAddBranch}><Plus size={13} color={theme.textMuted} strokeWidth={1.8} /><span className={"" + " " + ""}>新建分支</span></button></>}</div>}
      </div>}
    </div>;
}
