...
export function ContextDropdown({
  ...
  const renderControl = (kind: Exclude<DropdownKind, null>, icon: ReactNode, label: string, labelText: string, disabled = false) => <button className="flex h-7 min-w-0 items-center gap-1.5 rounded-md px-2 text-xs text-text-secondary hover:bg-hover disabled:opacity-40" onClick={() => onToggle(kind)} disabled={disabled} role="button" aria-label={labelText}>
  ...
  return <div className="flex min-w-0 items-center gap-1">
      <div className="relative min-w-0">
        ...
        {open === 'project' && <div role="menu" aria-label="Project selection" className="absolute bottom-full left-0 z-50 mb-1 min-w-40 rounded-md border border-border bg-card p-1 shadow-xl">
          <div className="flex flex-col">
            ...
            return <button className="flex h-7 w-full items-center justify-between gap-1.5 rounded px-1.5 text-left text-xs hover:bg-hover" key={workspace.id} onClick={() => onSelectProject(workspace.id)} role="menuitem">
                <span className="truncate">{workspace.title}</span>
                {active && <Check size={13} color={theme.accentColor} strokeWidth={2} />}
              </button>;
          })}
          </div>
          <button className="flex h-7 w-full items-center gap-1.5 rounded px-1.5 text-left text-xs text-text-secondary hover:bg-hover" onClick={onAddWorkspace} role="menuitem" aria-label="添加新项目">
        ...
        {open === 'environment' && <div role="menu" aria-label="Environment selection" className="absolute bottom-full left-0 z-50 mb-1 min-w-44 rounded-md border border-border bg-card p-1 shadow-xl">
          <div className="flex flex-col">
            ...
            return <button className="flex min-h-8 w-full items-center justify-between gap-1.5 rounded px-1.5 text-left hover:bg-hover" key={server.id} onClick={() => void onSelectServer(server)} role="menuitem">
                <span className="flex min-w-0 items-center gap-1.5"><Globe size={12} color={active ? theme.accentColor : theme.textMuted} strokeWidth={1.8} /><span className="flex min-w-0 flex-col"><span className="truncate text-xs">{server.name}</span><span className="truncate text-[10px] text-text-secondary">{server.address}</span></span></span>
      ...
      {isGitRepo && <div className="relative min-w-0">
        ...
        {open === 'branch' && <div role="menu" aria-label="Branch selection" className="absolute bottom-full left-0 z-50 mb-1 min-w-40 rounded-md border border-border bg-card p-1 shadow-xl">
          {branchesLoading && !branches ? <div className="flex p-2"><span className="size-3 animate-spin" /></div> : localBranches.length === 0 ? <span className="block px-1.5 py-2 text-xs text-text-secondary">No branches found</span> : <><div className="flex flex-col">{localBranches.map(branch => <button className="flex h-7 w-full items-center justify-between gap-1.5 rounded px-1.5 text-left text-xs hover:bg-hover" key={branch.name} onClick={() => void onSelectBranch(branch)} role="menuitem"><span className="flex items-center gap-1.5"><GitBranch size={12} color={branch.is_current ? theme.accentColor : theme.textMuted} strokeWidth={1.8} /><span>{branch.name}</span></span>{busy === branch.name ? <span className="size-3 animate-spin" /> : branch.is_current && <Check size={12} color={theme.accentColor} strokeWidth={2} />}</button>)}</div><button className="flex h-7 w-full items-center gap-1.5 rounded px-1.5 text-left text-xs text-text-secondary hover:bg-hover" onClick={onAddBranch}><Plus size={12} color={theme.textMuted} strokeWidth={1.8} /><span>新建分支</span></button></>}</div>}
      </div>}
    </div>;
}
