...
export function InputCard({
  ...
  return <div className={"" + " " + ""}>
      <input ref={inputRef} placeholder="Ask anything..." placeholderTextColor={theme.textMuted} className={"" + " " + "" + " " + (sendDisabled && !canComposeWhileDisabled ? "opacity-[0.5]" : "")} focusStyle={{
      outlineWidth: 0,
      borderWidth: 0,
      ...
      <div className={""}>
        ...
        <button className={""} onClick={handleFilePick} disabled={inputDisabled} role="button" aria-label="Attach file"><Plus size={18} color={theme.textMuted} strokeWidth={1.8} /></button>
        {isListening ? <button className={""} onClick={handleMicPress} role="button" aria-label="Stop recording"><Square size={12} color="#EF4444" strokeWidth={2} fill="#EF4444" /><WaveformBars audioLevel={audioLevel} /></button> : <button className={""} onClick={handleMicPress} disabled={inputDisabled} role="button" aria-label="Start voice input"><Mic size={16} color={theme.textMuted} strokeWidth={1.8} /></button>}
