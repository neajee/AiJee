...
export function ModelPicker({
  ...
  return <div className={"block"}>
      ...
      {isWideScreen && activeDropdown === 'model' && <div role="menu" aria-label="Model selection" className={"  bg-surface border-border opacity-100"}>
          <div className={" "}>
            <input ref={modelSearchRef} placeholder="Search models..." placeholderTextColor={theme.textMuted} className={"  text-foreground"} value={modelSearch} onChangeText={value => {
          ...
          <div ref={modelScrollRef} className={"block"} keyboardShouldPersistTaps="handled">
            ...
            return <button key={model.id} onClick={() => handleSelectModel(model.provider ?? 'unknown', model.id)} role="menuitem" aria-label={`${model.name ?? model.id} by ${model.provider ?? 'unknown'}`} accessibilityState={{
              selected: active
                  ...
                  <div className={"block"}><ProviderIcon provider={model.provider ?? 'unknown'} size={14} color={active ? theme.accentColor : theme.textMuted} /><span className={" "}>{model.name}</span></div>
