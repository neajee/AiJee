...
export function FileTree({
  ...
  const tree = <>
      <div className="shrink-0 px-1.5 pb-1 pt-1.5">
        <div className="flex h-7 items-center gap-1.5 rounded-md border border-border bg-surface-raised px-2">
          ...
          <input value={query} onChange={event => setQuery(event.target.value)} placeholder="Filter files…" aria-label="Filter files" className="min-w-0 flex-1 bg-transparent text-xs text-foreground outline-none placeholder:text-text-tertiary" />
  ...
  return <div ref={containerRef} className="flex min-h-0 flex-1 flex-col">
    ...
    viewingFile ? <FileViewer filePath={viewingFile} rootPath={rootPath} onClose={() => onViewFile(null)} /> : <div className="flex min-h-0 flex-1 flex-col">{tree}</div>) : <div className="flex min-h-0 flex-1 flex-row">
          ...
          <div className="flex min-h-0 shrink-0 flex-col border-l border-border" style={{ width: treeWidth }}>
            {tree}
          </div>
        </div>}
    </div>;
