...
export function TasksPanelContent() {
  ...
  const [expandedSection, setExpandedSection] = useState<'available' | 'running' | null>(instances.length > 0 ? 'running' : 'available');
  useEffect(() => {
    if (workspaceId) {
  ...
  const handleStart = useCallback((label: string) => {
  ...
  const handleSelectTask = useCallback((label: string, instanceId?: string) => {
  ...
  return <div style={styles.list}>
      {instances.length > 0 && <>
          <button onClick={() => setExpandedSection(expandedSection === 'running' ? null : 'running')} style={styles.sectionHeader}>
          ...
          {expandedSection === 'running' && instances.map(instance => <TaskInstanceRow key={instance.id} instance={instance} isSelected={instance.label === selectedTaskLabel} onSelect={() => handleSelectTask(instance.label, instance.id)} onStop={() => handleStop(instance.id)} onRestart={() => handleRestart(instance.id)} onRemove={() => handleRemove(instance.id)} textPrimary={textPrimary} textMuted={textMuted} hoverBg={hoverBg} isDark={isDark} />)}
        </>}

      {availableTasks.length > 0 && <>
          <button onClick={() => setExpandedSection(expandedSection === 'available' ? null : 'available')} style={styles.sectionHeader}>
      ...
      {instances.length === 0 && availableTasks.length === 0 && <div style={styles.emptyState}>
