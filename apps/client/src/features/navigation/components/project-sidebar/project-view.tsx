...
export function ProjectSidebarView({
  ...
  return <div className="flex h-full w-full flex-col overflow-y-auto bg-background text-sm">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
        ...
        {rest.length === 0 && pinned.length === 0 ? <span className="px-2 py-3 text-xs text-text-tertiary">暂无项目</span> : rest.map(renderWorkspace)}
      </div>
      ...
      <AppModal visible={!!deleteWorkspace} onClose={() => setDeleteWorkspace(null)} title="删除项目？" showClose>
        <div className="flex flex-col gap-2">
          <p className="text-sm text-foreground">“{deleteWorkspace?.title}” 将从 AiJee 移除。</p>
          <p className="text-xs text-muted-foreground">本地目录和其中的对话文件不会被删除。</p>
          <div className="mt-4 flex justify-end gap-2"><button className="rounded-md px-3 py-2 text-sm hover:bg-hover" onClick={() => setDeleteWorkspace(null)}>取消</button><button className="rounded-md bg-error px-3 py-2 text-sm text-error-content hover:opacity-90" onClick={confirmDelete}>删除项目</button></div>
        </div>
      </AppModal>
    </div>;
}
