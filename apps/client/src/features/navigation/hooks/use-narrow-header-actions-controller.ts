...
export function useNarrowHeaderActionsController({
  visible,
  onClose,
  items
}: NarrowHeaderActionsSheetProps): NarrowHeaderActionsSheetViewProps {
  ...
  const panGesture = Gesture.Pan().onUpdate(event => {
    ...
    if (event.translationY > 100 || event.velocityY > 500) runOnJS(dismiss)();else translateY.value = withTiming(0, TIMING_CONFIG);
  });
