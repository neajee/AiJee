...
export function useServersController() {
  ...
  const handleRefreshCode = useCallback(async () => {
    ...
    try {
      ...
      const authorized = await useAuthStore.getState().authorizeWithCode(server.address, result.code, server.id, server.name);
  ...
  const handleConnect = useCallback(async (server: Server) => {
    try {
      ...
      const connected = auth.hasToken(server.id) ? await auth.activateServer(server) : false;
      if (connected) {
  ...
  const handleSave = useCallback(async (data: ServerFormData) => {
