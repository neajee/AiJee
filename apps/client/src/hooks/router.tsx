...
import { Link as TanStackLink, Outlet, useLocation, useNavigate, useRouterState } from "@tanstack/react-router";
...
export function useRouter() {
  ...
  return {
    navigate: (target: NavigationTarget) => navigate({
      to: toPath(target)
    }),
    push: (target: NavigationTarget) => navigate({
      to: toPath(target)
    }),
    replace: (target: NavigationTarget) => navigate({
      to: toPath(target),
      replace: true
    }),
...
export function useLocalSearchParams<T extends Record<string, string>>(): Partial<T> {
  return (useRouterState as any)({
    select: (state: any) => Object.assign({}, ...state.matches.map((match: any) => match.params))
  }) as Partial<T>;
}
