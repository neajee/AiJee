...
import { ArrowLeft, Link2, QrCode } from 'lucide-react-native';
  ...
  return <View style={[styles.screen, { backgroundColor: colors.background }]}>
    <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 24 }]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      <View style={styles.header}>
        <Pressable accessibilityLabel="返回" onPress={onClose} style={({ pressed }) => [styles.iconButton, { backgroundColor: colors.surfaceRaised, borderColor: colors.borderStrong }, pressed && styles.pressed]}><ArrowLeft color={colors.text} size={19} /></Pressable>
      ...
      <View style={[styles.card, { backgroundColor: colors.surfaceRaised, borderColor: colors.borderStrong }]}>
        <View style={[styles.qrIcon, { backgroundColor: colors.accent }]}><QrCode color={colors.onAccent} size={24} /></View>
        ...
        <View style={[styles.inputRow, { backgroundColor: colors.surface, borderColor: colors.borderStrong }]}>
          <Link2 color={colors.textTertiary} size={18} />
