...
export default function MobileServersScreen() {
  ...
  const connectRoute = process.env.EXPO_OS === 'web' ? '/mobile-preview-scan-connect' : '/scan-connect';
  return <View style={[styles.screen, { backgroundColor: colors.background }]}>
    ...
    <View style={styles.actions}>
      <Pressable onPress={() => router.push(connectRoute as never)} style={({ pressed }) => [styles.outline, { borderColor: colors.borderStrong }, pressed && styles.pressed]}><QrCode color={colors.textSecondary} size={16} /><Text style={[styles.outlineText, { color: colors.text }]}>扫描授权码</Text></Pressable>
      <Pressable onPress={() => router.push(connectRoute as never)} style={({ pressed }) => [styles.primary, { backgroundColor: colors.text }, pressed && styles.pressed]}><Plus color={colors.background} size={17} /><Text style={[styles.primaryText, { color: colors.background }]}>添加服务器</Text></Pressable>
    </View>
  </View>;
}
