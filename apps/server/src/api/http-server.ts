...
export class AiJeeHttpServer {
  ...
  async marketplaceOperation(body: Record<string, unknown>): Promise<unknown> { return this.packages.enqueueOperation({ operation: String(body.operation ?? ""), name: String(body.name ?? ""), scope: String(body.scope ?? "user"), version: body.version == null ? null : String(body.version), cwd: typeof body.cwd === "string" ? body.cwd : undefined }, async () => { try { await this.runtime.reloadResources(); } catch {} }); }
